...
dlio::MapNode::MapNode(): Node("dlio_map_node") {
  ...
  this->keyframe_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    "keyframes", 10, [this](const sensor_msgs::msg::PointCloud2::SharedPtr msg) {this->callbackKeyframe(msg);}, keyframe_sub_opt);
  ...
  this->dlio_map.reset(new pcl::PointCloud<PointType>());
...
void dlio::MapNode::savePCD(std::shared_ptr<direct_lidar_inertial_odometry::srv::SavePCD::Request> req,
  ...
  pcl::PointCloud<PointType>::Ptr m(new pcl::PointCloud<PointType>(*this->dlio_map));
