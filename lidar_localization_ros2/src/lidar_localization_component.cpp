...
void PCLLocalization::initialPoseReceived(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
  ...
  // Only try to process the last scan *if* we have one
  if (last_scan_ptr_) {
    RCLCPP_INFO(get_logger(), "initialPoseReceived: processing last scan");
    cloudReceived(last_scan_ptr_);
  } else {
    RCLCPP_INFO(get_logger(), "initialPoseReceived: no previous scan available, waiting for next cloud");
  }
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
      "cloudReceived: received null msg pointer, skipping");
  return;
}
